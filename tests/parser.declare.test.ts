...
describe('T-SQL Parser - DECLARE', () => {
    ...
    test('multiple variable declarations', () => {
        ...
        expect(stmt.variables[1].name)
            .toBe('@Name');

        expect(stmt.variables[1].dataType)
            .toBe('VARCHAR(100)');
    });
    ...
    test('table variable declaration', () => {
        ...
        expect(stmt.variables[0].columns)
            .toHaveLength(2);
    });
    ...
    test('table variable with PRIMARY KEY', () => {
        ...
        expect(cols[0].constraints)
            .toHaveLength(1);

        expect(
            cols[0].constraints![0].kind
        ).toBe('PRIMARY KEY');
    });
    ...
    test('multiple DECLARE statements without semicolon', () => {
        ...
        expect(
            first.variables[0].name
        ).toBe('@Id');

        expect(
            second.variables[0].name
        ).toBe('@Id2');
