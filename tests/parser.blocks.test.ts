...
describe('T-SQL Parser - IF / ELSE / BEGIN / END', () => {
    ...
    describe('THEN branches', () => {
        ...
        test('BEGIN END block with multiple statements', () => {
            ...
            expect(
                stmt.thenBranch.body
            ).toHaveLength(3);

            expect(
                stmt.thenBranch.body[0].type
            ).toBe('SelectStatement');

            expect(
                stmt.thenBranch.body[1].type
            ).toBe('SetStatement');

            expect(
                stmt.thenBranch.body[2].type
            ).toBe('PrintStatement');
        });
    ...
    describe('ELSE branches', () => {
        test('ELSE with single statement', () => {
            ...
            expect(stmt.elseBranch)
                .toBeDefined();

            expect(
                stmt.elseBranch.type
            ).toBe('SelectStatement');
        });
    ...
    describe('offset tracking', () => {
        test('IF node has valid offsets', () => {
            ...
            expect(stmt.start)
                .toBeLessThan(stmt.end);

            expect(stmt.condition.start)
                .toBeGreaterThanOrEqual(stmt.start);

            expect(stmt.thenBranch.end)
                .toBeLessThanOrEqual(stmt.end);
        });
        ...
        test('BEGIN END block has valid offsets', () => {
            ...
            expect(stmt.body[0].start)
                .toBeGreaterThan(stmt.start);
        });
    });
