...
describe('position helpers', () => {
    ...
    test('converts positions to offsets for LF text', () => {
        ...
        expect(positionToOffset(sql, { line: 1, character: 2 })).toBe(
            sql.indexOf('Id')
        );
        expect(positionToOffset(sql, { line: 2, character: 0 })).toBe(
            sql.indexOf('FROM')
        );
    });
    ...
    test('handles CRLF line endings without exposing newline characters', () => {
        ...
        expect(index.positionToOffset({ line: 1, character: 2 })).toBe(
            sql.indexOf('Id')
        );
    ...
    test('clamps out-of-range offsets and positions', () => {
        ...
        expect(positionToOffset(sql, { line: 99, character: 99 })).toBe(
            sql.length
        );
    });
