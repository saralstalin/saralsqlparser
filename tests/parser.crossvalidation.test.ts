...
describe('T-SQL Parser - Cross Validation / Real World Quirks', () => {
    ...
    test('EXISTS and NOT EXISTS parse correctly', () => {
        ...
        expect(
            second.where.operator
        ).toBe('NOT');

        expect(
            second.where.right.type
        ).toBe('ExistsExpression');
    });
    ...
    test('transaction statements parse successfully', () => {
        ...
        expect(body[0].type)
            .toBe('TransactionStatement');

        expect(body[1].type)
            .toBe('UpdateStatement');
    ...
    test('large real-world mixed workload parses under 60ms with zero issues in built JS', () => {
        ...
        expect(result.ast.body.length)
            .toBeGreaterThan(500);

        for (const stmt of result.ast.body) {

            expect(stmt.type)
                .not.toBe('ErrorStatement');

            expect(
                (stmt as any).incomplete
            ).toBeUndefined();
        }
    });
});
