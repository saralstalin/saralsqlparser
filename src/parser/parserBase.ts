...
import { RESYNC_KEYWORDS, STRUCTURAL_KEYWORDS } from './grammar';
...
export abstract class ParserBase {
    ...
    protected consume() { return this.tokens[this.pos++]; }
    ...
    protected getCompoundAssignmentBinaryOperator(
        operator: string
    ): string | null {
        switch (operator) {
            case '+=': return '+';
    ...
    protected isIdentifierListBoundary(
        token?: Token
    ): boolean {
        return (
            !token ||
    ...
    protected recoverToStatementBoundary(
        extra: string[] = []
    ) {
        this.recoverTo([
            ';',
